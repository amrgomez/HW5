...
app.config["SQLALCHEMY_DATABASE_URI"] =  os.environ.get('DATABASE_URL') or "postgresql://localhost/amrgomezHW5"
## Provided:
...
class Priorities(FlaskForm):
    items= FloatField("Rank the priority of this item", validators= [Required()])
    submit= SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('all_lists'))
    return render_template('update_item.html', form=form, items=items)
